...
def model_data_splitting(tattoos_meta_data_processed_augmented, val_perc=0.1, test_perc=0.1, dev=False):
    """Split model data into training & test sets when run in prod, and training,  valiation & test sets
    when run in dev. The function tries to maintain the split percentages for all data as well as
    ...
    Args:
        ...
        dev (bool, optional): To indicate dev or prod run.
    ...
    for row in syle_tattoo_counts.sort_values('tattoo_id', ascending=True).iterrows():
        ...
        if dev:
            ...
            style_test_ids = np_repr.choice(tattoo_ids_for_style_left['tattoo_id'].values, size = test_set_size_left, replace=False)
        ...
        if dev:
            ...
            train_set_ids = train_set_ids.union(
            set(tattoo_ids_for_style_left['tattoo_id'].values).difference(set(style_test_ids))
            )
