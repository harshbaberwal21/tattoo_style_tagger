...
def process_tattoo_meta_data_to_long_form(tattoo_images_meta_data):
    ...
    tattoo_images_meta_data["styles"] = tattoo_images_meta_data[
        "styles"].str.replace('[', "").str.replace(']', "").str.split(",")

    tattoo_meta_data_exploded = tattoo_images_meta_data.explode(column="styles")
    tattoo_meta_data_exploded["styles"] = tattoo_meta_data_exploded['styles'].str.strip()
    tattoo_meta_data_exploded["styles"] = tattoo_meta_data_exploded["styles"].str.replace("'", "")
...
def filter_out_tattoos(
        tattoo_meta_data_exploded,
        style_labels_count_threshold=7):
    ...
    tattoo_urls_to_consider = tattoo_images_style_counts[
        tattoo_images_style_counts['styles']<=style_labels_count_threshold]['image_url'].values


    tattoo_meta_data_exploded_filtered = tattoo_meta_data_exploded[
...
def handle_duplicates(tattoo_meta_data_exploded_filtered):
    ...
    new_tattoo_ids = tattoo_meta_data_exploded_filtered[
        ['image_url']
    ...
    tattoo_meta_data_exploded_filtered.rename(columns={'tattoo_id':'orig_tattoo_id'}, inplace=True)
